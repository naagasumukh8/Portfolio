  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      timelineData.forEach((item, index) => {
        ...
        nodeEl.style.transform = `translate(${x.toFixed(2)}px, ${y.toFixed(2)}px)`;
        nodeEl.style.zIndex = String(zIndex);
        nodeEl.style.opacity = String(opacity.toFixed(3));
      });
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 animate-pulse flex items-center justify-center z-10">
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              <div
                ...
                className={`absolute cursor-pointer ${autoRotate ? "transition-none" : "transition-all duration-700"}`}
                style={nodeStyle}
                onClick={(e) => {
                  e.stopPropagation();
                  toggleItem(item.id);
